...
function selectOrderTradeStatus() {
...
const endPointCall = "fapi" // alter -> dapi sapi
const securePort = 'https://'
const trade_api_url = `${securePort}${endPointCall}.binance.com`
...
function mbxSignatureSign(qs) {
    $.ajaxSetup({
        headers: { "X-MBX-APIKEY": $('#api_key').val(), },
        contentType: "application/json; charset=utf-8",
    })

    const signature = mbxSign.encp1228(qs, $('#api_secret_key').val())//.digest('hex')
...
function checkExchangeInfoAndPercentMarket() {
    ...
    $.ajax({
        type: 'GET',
        url: `${trade_api_url}/${endPointCall}/v1/exchangeInfo`,
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            ...
            $.ajax({
                type: 'GET',
                url: `${trade_api_url}/${endPointCall}/v1/ticker/24hr`, // Not Validate Delist
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    if (Array.isArray(result)) {
                        result.forEach(({ symbol, priceChangePercent, lastPrice }) => {
                            if (symbol in positionListSymbolKey) {
                                ...
                                if (symbol.includes('USDT') && arr_delist_or_no_trade.indexOf(symbol) === -1) {
                                    $('#tbody_percent_change_24h').append(`
                                    <tr>
                                        <td>${symbol}</td><!--<td>${lastPrice}</td>--><td>${parseFloat(priceChangePercent).toFixed(1)} %</td>
                                        <td class="flex-center">Size:&nbsp;<input id="size_usdt_${symbol}" style="width: 40px; height: 20px; padding: 2px;" type="number" value="${defaultMargin}"/>&nbsp;$&nbsp;
                                        <button disabled style="height: 22px; font-size: 12px;" id="btn_trade_${symbol}" onclick="trade('${symbol}', '${lastPrice}')">Trade</button>
                        ...
                        $('#table_percent_change_1D').dataTable({
...
function checkAvailableBalance(cb = () => { }) {
    ...
    $.ajax({
        type: 'GET',
        url: `${trade_api_url}${endPoint}${qs}&signature=${mbxSignatureSign(qs)}`,
        success: function (result) {
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                if ((existLong && !existShort) && positionSide === "LONG") {
                    $(`#btn_trade_${symbol}`).html("<b style='color: black;'>Longing.</b>")
                        .prop("disabled", true)
                        .css({ 'background-color': 'lime', 'color': 'black !important' })
                ...
                if ((!existLong && existShort) && positionSide === "SHORT") {
                    $(`#btn_trade_${symbol}`).html("<b style='color: black;'>Shorting.</b>")
                        .prop("disabled", true)
                        .css({ 'background-color': 'red', 'color': 'black !important' })
                ...
                if (existLong && existShort) {
                    $(`#btn_trade_${symbol}`).html("<b style='color: black;'>Hedging</b>")
                        .prop("disabled", true)
                        .css({ 'background-color': 'yellow', 'color': 'black !important' })
                ...
                if (!existLong && !existShort) {
                    $(`#btn_trade_${symbol}`).html("<b style='color: black;'>Trade</b>")
                        .prop("disabled", false)
...
function trade(symbol, lastPrice) {
    ...
    const startTrade = () => {
        ...
        const encodeOpenOrder = encodeURIComponent(JSON.stringify(batchOrderOpenHedge))
        ...
        $.ajax({
            type: 'POST',
            url: `${trade_api_url}${endpoint3}${qs3}&signature=${mbxSignatureSign(qs3)}`,
            success: function (result) {
                ...
                */
                ...
                if (Array.isArray(result)) {
                    ...
                    if (isSuccessOpenHedge) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: `${trade_api_url}${endpoint3}${qs4}&signature=${mbxSignatureSign(qs4)}`,
                            success: function (result) {
    ...
    const setLeverage = () => {
        if (currentLeverage != autoLeverage) {
            ...
            const endPoint2 = `/${endPointCall}/v1/leverage?`
            const ts2 = Date.now()
            const qs2 = `symbol=${symbol}&leverage=${autoLeverage}&timestamp=${ts2}`

            $.ajax({
                type: 'POST',
                url: `${trade_api_url}${endPoint2}${qs2}&signature=${mbxSignatureSign(qs2)}`,
                success: function (result) {
                    startTrade()
                },
                error: function (err) {
    ...
    const setMarginType = () => {
        if (!isolated) {
            ...
            const endPoint1 = `/${endPointCall}/v1/marginType?`
            const ts1 = Date.now()
            const qs1 = `symbol=${symbol}&marginType=ISOLATED&timestamp=${ts1}`

            $.ajax({
                type: 'POST',
                url: `${trade_api_url}${endPoint1}${qs1}&signature=${mbxSignatureSign(qs1)}`,
                success: function (result) {
                    setLeverage()
                },
                error: function (err) {
